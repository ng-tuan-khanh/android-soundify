package com.ngtuankhanh.soundify.ui.yourlibrary

import android.os.Bundle
import androidx.fragment.app.Fragment
import android.view.LayoutInflater
import android.view.View
import android.view.ViewGroup
import androidx.lifecycle.lifecycleScope
import androidx.navigation.fragment.findNavController
import androidx.recyclerview.widget.LinearLayoutManager
import com.ngtuankhanh.soundify.databinding.FragmentYourLibraryBinding
import com.ngtuankhanh.soundify.ui.activities.BaseActivity
import com.ngtuankhanh.soundify.ui.adapters.YourLibraryItemsAdapter
import com.ngtuankhanh.soundify.ui.models.LibraryItemType
import kotlinx.coroutines.launch

class YourLibraryFragment : Fragment() {
    private lateinit var binding: FragmentYourLibraryBinding

    private val adapter by lazy {
        YourLibraryItemsAdapter() { item ->
            when (item.type) {
                LibraryItemType.Playlist -> {
                    val action =
                        YourLibraryFragmentDirections.actionLibraryFragmentToPlaylistDetailFragment(
                            item.id
                        )
                    this.findNavController().navigate(action)
                }

                LibraryItemType.Artist -> {
                    val action =
                        YourLibraryFragmentDirections.actionLibraryFragmentToArtistProfileFragment(
                            item.id
                        )
                    this.findNavController().navigate(action)
                }
                // TODO: Missing handler for remaining cases
                else -> {

                }
            }
        }
    }

    override fun onCreateView(
        inflater: LayoutInflater,
        container: ViewGroup?,
        savedInstanceState: Bundle?
    ): View? {
        binding = FragmentYourLibraryBinding.inflate(inflater, container, false)

        // Set up the view model
        val viewModelFactory = YourLibraryViewModel.Factory(requireActivity() as BaseActivity)
        val viewModel = viewModelFactory.create(YourLibraryViewModel::class.java)

        // Set up RecyclerView
        binding.yourLibraryItemsRecyclerView.layoutManager =
            LinearLayoutManager(context)
        binding.yourLibraryItemsRecyclerView.adapter = adapter

        // Observe data changes
        viewLifecycleOwner.lifecycleScope.launch {
            viewModel.yourLibraryItems.collect {
                adapter.submitList(it)
            }
        }

        return binding.root
    }

    var url: String =
        "data:image/png;base64,"  // This is empty right now, ensure you fill it with the appropriate base64 string or URL you want to use

//    private fun generateDummyLibraryItems(): List<YourLibraryItem> {
//        val items = mutableListOf<YourLibraryItem>()
//        // Thêm một số dummy data cho mục đích demo
//        for (i in 1..15) {
//            val item = YourLibraryItem().apply {
//                id = i.toString()
//                name = "Item $i"
//                type =
//                    LibraryItemType.values()[i % 3] // sẽ trả về một trong PLAYLIST, ALBUM, hoặc ARTIST
//                avatarImage = Image(url = this@YourLibraryFragment.url)
//            }
//            items.add(item)
//        }
//        return items
//    }
}
